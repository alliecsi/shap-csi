...
# Use float16 or mixed precision
...
class Gradient(Explainer):
    ...
    def __call__(self, X, nsamples=50):
    ...
    def shap_values(self, X, nsamples=50, ranked_outputs=None, output_rank_order="max", rseed=None, return_variances=False):
    ...
    def prepare_background_cache(self, X, nsamples=50, rseed=None):
        return self.explainer.prepare_background_cache(X, nsamples, rseed)
...
class _TFGradient(Explainer):
    ...
    def __init__(self, model, data, session=None, batch_size=50, local_smoothing=0):
        ...
        self._delta_cache = {} # Cache for background deltas
        self._grad_cache = {} # Cache for gradients
    ...
    def prepare_background_cache(self, X, nsamples=50, rseed=None):
        if hasattr(self, '_cached_samples') and self._cached_samples['nsamples'] == nsamples:
            return
        ...
        rinds = np.random.choice(self.data[0].shape[0], size=(X[0].shape[0], nsamples))
        ts = np.random.uniform(size=(X[0].shape[0], nsamples))
    ...
    def shap_values(self, X, nsamples=50, ranked_outputs=None, output_rank_order="max", rseed=None, return_variances=False):
        rinds = self._cached_samples['rinds']
        ts = self._cached_samples['ts']


        # Determine how many outputs we're explaining to conditionally enable caching
        n_outputs_to_explain = (
            ranked_outputs if isinstance(ranked_outputs, int) else
            ranked_outputs.shape[1] if isinstance(ranked_outputs, np.ndarray) else
            self.model_output.shape[1] if self.multi_output else 1
        )
        
        # Enable delta caching if >1 outputs and no local smoothing
        should_cache_deltas = self.local_smoothing == 0 and n_outputs_to_explain > 1
        # Enable gradient caching if >1 outputs and no local smoothing
        should_cache_grads  = self.local_smoothing == 0 and n_outputs_to_explain > 1

        if should_cache_deltas:
            print("CACHING DELTAS")
        else:
            print("NOT CACHING DELTAS")

        if should_cache_grads:
            print("CACHING GRADS")
        else:
            print("NOT CACHING GRADS")
        ...
        for i in range(model_output_ranks.shape[1]):
            ...
            phis = [np.zeros(X[l].shape, dtype=np.float32) for l in range(len(X))]
            phi_vars = [np.zeros(X[l].shape, dtype=np.float32) for l in range(len(X))]

            for j in range(X[0].shape[0]):
                cache_key = (id(X[0]), j, nsamples) # Delta cache key based on sample and nsamples
                use_cache = should_cache_deltas and cache_key in self._delta_cache

                if use_cache:
                    cached_deltas = self._delta_cache[cache_key]
                    for l in range(len(X)):
                        x = X[l][j]
                        x_expanded = np.broadcast_to(x, (nsamples,) + x.shape)
                        background = self.data[l][rinds[j]]
                        t = ts[j][:, np.newaxis, np.newaxis]
                        samples_input[l][:] = t * x_expanded + (1 - t) * background
                        samples_delta[l][:] = cached_deltas[l]
                else:
                    current_deltas = []
                    for l in range(len(X)):
                        if self.local_smoothing > 0:
                            ...
                            x = X[l][j]
                        x_expanded = np.repeat(x[np.newaxis, :], nsamples, axis=0)
                        background = self.data[l][rinds[j]]
                        t = ts[j][:, np.newaxis, np.newaxis]
                        samples_input[l][:] = t * x_expanded + (1 - t) * background
                        delta = x - background
                        samples_delta[l][:] = delta
                        current_deltas.append(delta)

                    if should_cache_deltas: # Write to delta cache only if caching is enabled
                        self._delta_cache[cache_key] = current_deltas

                # cached-aware:
                find = model_output_ranks[j, i]
                grad_cache_key = (id(X[0]), j, find, nsamples) # Gradient cache key based on input, output, and nsamples

                if should_cache_grads and grad_cache_key in self._grad_cache:
                    grad = self._grad_cache[grad_cache_key]
                else:
                    grads = []
                    ...
                    if should_cache_grads: # Write to grad cache only if caching is enabled
                        self._grad_cache[grad_cache_key] = grad
    ...
    def clear_cache(self):
        self._delta_cache.clear() # Clear background delta cache
        self._grad_cache.clear() # Clear gradient cache
        if hasattr(self, "_cached_samples"): # Clear cached samples if needed
